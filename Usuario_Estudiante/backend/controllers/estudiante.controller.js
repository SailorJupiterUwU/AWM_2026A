    ...
    if (!nombre || !edad)
        ...
        if (estudianteFound)
            ...
            Estudiante.create({ nombre, edad, url})
                .then(estudianteNuevo => response.json({ nombre: estudianteNuevo.nombre, edad: estudianteNuevo.edad, url: estudianteNuevo.url }))
                .catch(err => response.status(500).json(err))
        }
    }
...
module.exports.editEstudiante = (request, response) => {
    ...
    Estudiante.findOneAndUpdate({ _id: id }, { nombre, edad, url }, { returnDocument: 'after' })
        .then(estudianteEditado => response.json({nombre: estudianteEditado.nombre, edad: estudianteEditado.edad, url: estudianteEditado.url}))
        .catch(err => response.json(err))
}
