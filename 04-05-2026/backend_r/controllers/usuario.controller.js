...
module.exports.newUsuario = async (request, response) => {
    ...
    if (!email || !rol || !password) {
        return response.status(400).json({ message: "Todos los campos son obligatorios (boink)" });
    }
    ...
    try {
        ...
        if (usuarioFound) {
            return response.status(400).json({ message: "El usuario ya existe owo (menso) (boink)" });
        }
        ...
        const usuarioNuevo = await Usuario.create({ email, rol, password: hashedPassword });
        response.json({ email: usuarioNuevo.email, rol: usuarioNuevo.rol });
    } catch (err) {
...
module.exports.loginUsuario = async (request, response) => {
    ...
    try {
        ...
        if (usuarioFound && (await bcrypt.compare(password, usuarioFound.password))) {
            response.json({
                message: "Loggeado Exitosamente OwO",
                token: generateToken(usuarioFound.id, usuarioFound.rol)
            });
        } else {
